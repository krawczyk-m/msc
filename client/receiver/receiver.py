...
class Receiver(object):
    ...
    def send_notify_ack(self):
        print "Received notify. Sending NOTIFY_ACK"
    ...
    def send_report(self):
        print "Received steganogram: {}. Sending REPORT".format(reduce(lambda x, y: str(x) + str(y), self.steganogram, ""))
        self.messenger.notify(Messages.REPORT)

    def recvd_report_ack(self):
        message = self.messenger.receive()
        if message == Messages.REPORT_ACK:
            print "Received REPORT_ACK. Ending conversation"
            return True
        return False

    def _nfqueue_receive(self):
        self.nfqueue = NetfilterQueue()
        print "Listening on {}".format(self.listen_queue_num)
    ...
    def _handle_incoming_packets(self, pkt):
        ...
        print "Handling incoming packet with IP id: {}".format(ip_packet[IP].id)
